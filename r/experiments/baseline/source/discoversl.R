...
customDataDiffExp <- function(gene1,mymutdata,myexpression){
  ...
  tryCatch(
    ...
    dataexpr <- round(data.matrix(dataexpr[,-1]))
    colnames(dataexpr) <- sapply(colnames(dataexpr), function(x) substr(x, 1, 16))
    genemut <- subset(datam1, gene_symbol==gene1 & mutation_type==1, select=case_id)
    genemut <- sapply(genemut, function(x) substr(x, 1, 16))
    genenonmut <- unique(colnames(dataexpr))
    if(length(genemut)>=1&&length(genenonmut)>=1){
      genemut <- unique(genemut)
      genenonmut <- unique(genenonmut)
      genenonmut <- setdiff(genenonmut, genemut)
      if(length(genemut)>=1 && length(genenonmut)>=1){
        mut <- genemut
        nonmut <- genenonmut
        mutexpr <- subset(dataexpr, select=colnames(dataexpr) %in% mut)
        nonmutexpr <- subset(dataexpr, select=colnames(dataexpr) %in% nonmut)
