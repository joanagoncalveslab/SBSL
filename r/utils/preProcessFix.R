...
preProcessFix <- function(x, method = c("center", "scale"),
                               thresh = 0.95,
                               pcaComp = NULL,
                               na.remove = TRUE,
                               k = 5,
  ...
  if(!inherits(x, "matrix") & !inherits(x, "data.frame"))
    stop("Matrices or data frames are required for preProcessFixing", call. = FALSE)
  ...
  out <- structure(out, class = "preProcessFix")
  out
}

#' @rdname preProcessFix
#' @importFrom stats complete.cases
#' @export
predict.preProcessFix <- function(object, newdata, ...) {
  ...
  if(!is.null(object$method$remove)) {
    ...
    if(ncol(newdata) == 0)
      stop("All predctors were removed as determined by `preProcessFix`")
  }
...
#' @export
print.preProcessFix <- function(x, ...) {
  #   printCall(x$call)
